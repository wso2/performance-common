...
def format_query(q):
    if q == 1:
        return "Query 1 (Operation count: 1 | Query depth: 2 | Query size: 157B | Response size: 790B)"
    if q == 2:
        return "Query 2 (Operation count: 4 | Query depth: 2 | Query size: 1KB | Response size: 5KB)"
    if q == 3:
        return "Query 3 (Operation count: 9 | Query depth: 2 | Query size: 2KB | Response size: 27KB)"
    return str(q)
...
def save_multi_columns_categorical_charts(df, chart, columns, y, hue, title, single_statistic=False,
    ...
    g = sns.histplot(x="Concurrent Users", y=y,
                       hue=hue, col=col,
